...
| `name` * | String | | Server name for use in logging. Is _REQUIRED_ if you do not provide a `log` argument, as `buildServer` expects one of the two. |
...
| | | | When passed, output is sent to a file in the specified directory with the syntax `[logPath].log`. The log file is rotated out once per day, and 3 days of logs plus the current day are held as per the [Bunyan spec](https://github.com/trentm/node-bunyan#stream-type-rotating-file). |
