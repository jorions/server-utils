...
const logInfo = log => async (ctx, next) => {
  const {
    request: {
      method,
      url,
      body: { password, token: reqToken, ...cleanBody },
      ip,
    },
    response,
    state: { id, start },
  ...
  body = truncate(body, state.requestBodyMaxLen)

  let { body: cleanResponseBody } = response
  if (cleanResponseBody && typeof cleanResponseBody === 'object') {
    ...
    cleanResponseBody = truncate(cleanResponseBody, state.responseBodyMaxLen)
  }
